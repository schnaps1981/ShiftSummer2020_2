...
class CitiesNetworkDataSourceImpl(val api: CitiesApi): CitiesNetworkDataSource {
    ...
    override suspend fun deleteById(id: Long) = apiCallWrapper { api.deleteById(id) }
    override suspend fun update(city: CityDto, id: Long) = apiCallWrapper { api.update(city, id) }
}
